...
impl LocalhostBitcoinNode {
    pub fn new(bitcoind_api: Url) -> LocalhostBitcoinNode {
        Self {
            bitcoind_api,
            wallet_name: "papaya_wallet".to_string(),
        }
    }
    ...
    pub fn create_empty_wallet(&self) -> Result<(), Error> {
    ...
    pub fn import_address(&self, address: &bitcoin::Address) -> Result<(), Error> {
    ...
    pub fn raw_to_utxo(raw: &Value) -> Result<UTXO, Error> {
        Ok(UTXO {
            txid: raw["txid"]
                .as_str()
                .ok_or(Error::InvalidResponseJSON(
    ...
    pub fn generate_to_address(&self, address: &str) -> Result<serde_json::Value, Error> {
        let params = (
            101,
            address.clone()
        );
